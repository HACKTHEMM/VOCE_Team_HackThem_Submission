...
async def get_audio_file(session_id: str):
    ...
    # Return the file with appropriate headers for audio playback
    return FileResponse(
...
async def get_latest_response(session_id: str):
    ...
    print(f"   └─ Text length: {len(response_data.get('text', ''))} characters")
    print(f"   └─ Audio file: {audio_file_path}")
    print(f"   └─ Audio filename: {response_data.get('audio_filename', 'N/A')}")
    ...
    if session_id in session_responses:
        ...
        if audio_file_path:
            ...
            if os.path.exists(audio_file_path):
                debug_info["file_checks"]["original_path"]["size"] = os.path.getsize(audio_file_path)
                debug_info["file_checks"]["original_path"]["modified"] = time.ctime(os.path.getmtime(audio_file_path))
...
def find_audio_file(audio_file_path):
    ...
    possible_paths.append(os.path.abspath(os.path.join(static_audio_dir, filename)))
